...
from utils import get_coreset_idx_randomp, get_tqdm_params
...
class KNNExtractor(torch.nn.Module):
    ...
    def __call__(self, x: tensor):
        with torch.no_grad():
            self.features = []
            self.feature_extractor(x.to(self.device))
        feature_maps = [fmap.to("cpu") for fmap in self.features]
        if self.pool:
...
class PatchCore(KNNExtractor):
    ...
    def fit(self, train_dl):
        ...
        print([feature_maps[i].shape for i in range(len(feature_maps))])
        ...
        torch.save(self.patch_lib,"./patch_lib")

    def predict(self, sample):		
        self.patch_lib = torch.load("./patch_lib")
        feature_maps = self(sample)
        largest_fmap_size = feature_maps[0].shape[-2:]
        self.resize = torch.nn.AdaptiveAvgPool2d(largest_fmap_size)
        resized_maps = [self.resize(self.average(fmap)) for fmap in feature_maps]
